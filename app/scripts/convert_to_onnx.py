...
def convert_to_onnx(
    ...
    try:
        ...
        try:
            ...
            raise ImportError("Transformers package not installed. Please install it: pip install transformers")
        
        # Check if torch is installed
        try:
            import torch
        except ImportError:
            raise ImportError("PyTorch package not installed. Please install it: pip install torch")
        
        # Check if onnx is installed
        try:
            import onnx
        except ImportError:
            raise ImportError("ONNX package not installed. Please install it: pip install onnx onnxruntime")
        ...
        try:
            ...
            onnx_inputs = {
                ...
                "token_type_ids": dummy_inputs.get("token_type_ids", torch.zeros_like(dummy_inputs["input_ids"])).numpy()
            }
            
            onnx_outputs = onnx_session.run(None, onnx_inputs)
            
            logger.info(f"ONNX inference successful, output shape: {onnx_outputs[1].shape}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Convert Hugging Face model to ONNX format")
    parser.add_argument("--model", type=str, help="Name or path of the model to convert")
    parser.add_argument("--output", type=str, help="Path to save the ONNX model")
    parser.add_argument("--quantize", action="store_true", help="Quantize the model for smaller size")
