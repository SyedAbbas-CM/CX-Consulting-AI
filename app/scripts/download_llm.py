...
def download_model(model_name: str, output_dir: str, force: bool = False) -> str:
    ...
    if model_name not in SUPPORTED_MODELS:
        logger.error(
            f"Model {model_name} not supported. Available models: {', '.join(SUPPORTED_MODELS.keys())}"
        )
        return None
    ...
    try:
        if model_info.get("direct_gguf", False):
            ...
            gguf_path = hf_hub_download(
                repo_id=repo_id, filename=filename, cache_dir=output_path
            )
            ...
            model_path = snapshot_download(repo_id=repo_id, cache_dir=output_path)
...
def convert_to_gguf(
    model_path: str, output_dir: str, model_name: str, quantization: str = "Q4_K_M"
) -> str:
    """
