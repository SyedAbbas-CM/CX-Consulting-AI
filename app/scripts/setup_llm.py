...
def check_environment():
    ...
    try:
        ...
        logger.warning("vLLM is not installed, consider installing it: pip install vllm")
    ...
    try:
        ...
        logger.warning("Ollama is not installed, consider installing it: https://ollama.ai/")
    ...
    try:
        ...
        if os.environ.get("HF_TOKEN"):
            ...
            logger.warning("No Hugging Face token found in environment, may have limited access")
    except ImportError:
...
def setup_model_ollama(model_id):
    ...
    try:
        ...
        if ollama_id in result.stdout:
            ...
            pull_result = subprocess.run(["ollama", "pull", ollama_id], capture_output=True, text=True)
...
def list_models():
    ...
    for model_id, model_info in LLM_OPTIONS.items():
        print("{:<10} {:<40} {:<10}".format(
            model_id,
            model_info["huggingface_id"],
            model_info["ollama_id"]
        ))
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Set up LLMs for use with vLLM and Ollama")
    parser.add_argument("--model", type=str, help="Model ID or Hugging Face ID")
    parser.add_argument("--backend", type=str, choices=["vllm", "ollama"], default="ollama", help="Backend to use (vllm or ollama)")
    parser.add_argument("--list", action="store_true", help="List available models")
    parser.add_argument("--check", action="store_true", help="Check environment")
    parser.add_argument("--model-name", type=str, help="Custom model name (for local models)")
