...
class AuthService:
    ...
    def create_user(
        ...
        try:
            ...
            cursor.execute(
                ...
                ''',
                (
                    user_id, username, email, hashed_password, full_name, company,
                    is_admin, now, now
                )
            )
    ...
    def update_user(self, user_id: str, user_data: Dict[str, Any]) -> Optional[dict]:
        ...
        try:
            ...
            for key, value in user_data.items():
                ...
                elif key in ["username", "email", "full_name", "company", "is_active", "is_admin"]:
    ...
    def verify_token(self, token: str) -> Optional[dict]:
        ...
        try:
            ...
            logger.debug(f"Token verified successfully. Payload: {payload}") # Log success
            return payload
        
        except jwt.ExpiredSignatureError:
            logger.warning("Token verification failed: Expired signature") # Log specific error
            return None
        except jwt.JWTError as e:
            logger.warning(f"Token verification failed: {type(e).__name__} - {str(e)}") # Log other JWT errors
            return None
        except Exception as e:
            logger.error(f"Unexpected error during token verification: {str(e)}", exc_info=True) # Log unexpected errors
            return None 