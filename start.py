...
def start_redis():
    ...
    try:
        ...
        logger.error("Could not import Redis manager. Make sure the app is properly installed.")
        return False

def start_app(dev_mode=False):
    """Start the FastAPI application."""
    try:
        logger.info(f"Starting CX Consulting AI in {'development' if dev_mode else 'production'} mode")
...
def main():
    ...
    parser.add_argument('--skip-redis', action='store_true', help='Skip Redis startup check')
